 ...
 *
 * <p>No ADR or design.md wording fixes an exact generation scheme —
 * design.md section 1.3 gives only the illustrative shape
 * {@code "job-abc123"}. This class picks the simplest scheme matching that
 * shape: a fixed {@code "job-"} prefix followed by a random UUID, which
 * guarantees practical uniqueness without any coordination (no counter, no
 * database round-trip before the job document itself is inserted).
 *
 ...
 * tests control outputs without bytecode manipulation").
 */
