...
* Bad, because a refresh-token flow is being requested (decision 12/acceptance criteria) alongside Client Credentials, which by OAuth2 specification (RFC 6749) does not mandate refresh tokens for the Client Credentials grant — Keycloak can be configured to issue one, but this is a Keycloak-side realm/client configuration choice (see Open question in Handoff below if the realm is not yet configured to do so), not an inherent property of the grant type itself. This must be explicitly configured on the Keycloak client used by the token-issuance realm.
