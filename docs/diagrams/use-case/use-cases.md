...
- No diagram-validation tooling applies to this repository (no `src/`, no build files yet — see `docs/diagrams/c4/context.md`, Limitations). Syntax was validated by manual inspection only (balanced brackets/parentheses for each shape type, valid `flowchart` direction keywords, no reserved characters left unescaped in labels).
